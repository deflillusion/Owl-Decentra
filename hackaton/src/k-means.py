...
segment_descriptions = {
    "Экономные локальные пользователи": "Преобладают снятия наличных, минимум валют и зарубежных операций.",
    "Активные премиум-пользователи": "Высокие суммы, много транзакций, часто зарубежные и мультивалютные.",
    "Стабильные локальные покупатели": "Очень высокая POS-активность, крупные суммы, мало валют.",
    "Глобальные путешественники": "Много валют, почти все операции зарубежные, активен ecom.",
    "Пассивные локальные пользователи": "Малые суммы, мало транзакций, только локальные операции.",
    "Спящие локальные пользователи": "Низкая активность, низкие доли по всем категориям.",
    "Активные автолюбители": "Высокая доля ecom, средние суммы, часть операций зарубежные.",
    "Элитные шопоголики": "Много валют, средние суммы, значительная часть зарубежных операций.",
    "Стабильные локальные шопоголики": "Средние суммы, высокая доля POS и шопинга, только локальные операции.",
    "Элитные глобальные путешественники": "Очень крупные суммы, часть операций зарубежные, высокая POS-активность.",
    # ... добавьте описания для других сегментов ...
}

final_results = client_features.copy()
final_results['segment_description'] = final_results['cluster_label'].map(segment_descriptions).fillna("Описание отсутствует")

# Сохраняем только нужные столбцы в parquet
final_results[['card_id', 'cluster_label', 'segment_description']].to_parquet('client_segments.parquet', index=False)
print("✅ Результаты сохранены в 'client_segments.parquet'")
